...
// Adapted by Niek Rutten, 17-5-2022
  ...
  while(true){
    ...
    if(debug){
      ...
      if (imeas%printfrac==0){
        ...
        if(abs(mean)>37){ //if disturbance is greater than 37, send signal to ESP
        digitalWrite(2,HIGH);
        metalState=1;
        }
        else{digitalWrite(2,LOW); // Reset every once in a while
        metalState=0;
        }
        if(imeas>200000){
